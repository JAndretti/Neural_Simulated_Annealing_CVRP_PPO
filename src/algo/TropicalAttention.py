...
class TropicalAttention(nn.Module):
    def __init__(
        ...
        self.out = nn.Linear(d_model, d_model, bias=False, device=device)

        if self.tropical_proj:
            # Multi-head attention tropical linear map
            self.query_trop = TropicalLinear(self.d_k, self.d_k, device=device)
            self.key_trop = TropicalLinear(self.d_k, self.d_k, device=device)
            self.value_trop = TropicalLinear(self.d_k, self.d_k, device=device)
    ...
    def forward(self, x):
        ...
        q = q.reshape(batch_size, seq_len, self.n_heads, self.d_k).permute(
            0, 2, 1, 3
        )  # [B, H, S, D]
        ...
        if self.symmetric:
            ...
            diff = q.unsqueeze(2) - k.unsqueeze(
                3
            )  # [B, S, 1, D] - [B, 1, S, D] -> [B, S, S, D]
